				...
				<div className="mt-8">
					...
					{posts.map((post) => (
						<div key={post.id} className="bg-gray-200 rounded p-4 mb-4">
							<p>{post.content}</p>
							<div className="mt-2">
								<input type="text" className="border border-gray-300 p-2 rounded focus:outline-none focus:border-blue-500" placeholder="Add a comment..." value={newComment} onChange={(e) => setNewComment(e.target.value)}/>
								<button className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded ml-2" onClick={() => handleCommentSubmit(post.id)}> <FontAwesomeIcon icon={faComment} className="mr-2" />Comment </button>
								<div className="mt-2">
